...
pub async fn files_commander(
    ...
    match cmd {
        ...
        FilesSubCommands::Ls { target } => {
            ...
            let (version, files_map, total) = match safe.fetch(&target_url, None).await? {
                ...
                SafeData::PublishedImmutableData {
                    ...
                    if let Some(file_item) = metadata {
                        ...
                        let xorurl_encoder = XorUrlEncoder::from_url(&resolved_from)?;
                        let cont_version = xorurl_encoder.content_version().unwrap_or_else(|| 0);
                        (cont_version, files_map, 1)
                    } else {
                        return Err(
